...
class MessageRequests extends React.Component {
    ...
    handleClose = () => {
        this.props.socket.emit('private message', this.state.acceptedMessage.socketId, '**'+this.state.userName + ' has left**');
        this.setState({inChat: false, acceptedMessage: {}});
        this.props.socket.emit('reject message', this.props.socket.id, this.state.acceptedMessage.socketId);
    };
